...
export const FilterTypes = (props: IFilterType) => {
  ...
  const getClassName = (name: KnowledgeType): string => "btn " + (name === props.filterType ? "btn-primary" : "")

  const renderButton = (name: KnowledgeType, key: number = -1) => (
    <span key={key} className={getClassName(name)} onClick={() => props.setFiltersType(name)} >
      {name}
    </span>
  )
